...
int32  ZAux_SearchEth(const char *ipaddress,  uint32 uims)
{
    ...
    for(int j= 0; j< 100;j++) //���100��IP�б�
    {
        ...
        ipos = sscanf(pstring , "%s", (char*)&buffer2);
        if(EOF  == ipos)
        {
            break;
        }
...
int32  ZAux_WriteUFile(const char *sFilename, float *pVarlist, int inum)
{
    ...
    if((int)fwrite(pVarlist, 4, inum, pfile) != inum)
	{
        //
...
Return:         //??????
*************************************************************/
int32  ZAux_GetFrameTrans2(ZMC_HANDLE  handle,int * axislist ,int axisnum, int tableinnum ,int tableoutnum ,int mode)
{
    char  cmd[2048] = {0};
    char  temp[2048] = {0};
    char  Ack[2048] = {0};
    ...
    {
        //
        sprintf(temp, "%d,",axislist[i]);
        strcat(cmd, temp);
    }
    sprintf(temp, "%d)",axislist[axisnum-1]);
    strcat(cmd, temp);

    strcat(cmd, "\n");
    //????????
    sprintf(temp, "FRAME_TRANS2(%d,%d,%d)",tableinnum,tableoutnum,mode);
    strcat(cmd, temp);
    //??????????��???
    return ZAux_DirectCommand(handle, cmd, Ack, 2048);
    return ERR_OK;
}
/*************************************************************
Description:    //����
